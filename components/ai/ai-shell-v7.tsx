...
import { FlowConfigPanelV7 } from "./flow-config-panel-v7";
import { ASSISTANT_SCRIPT, type AssistantBlock } from "@/lib/ai/mock-data";
import { cn } from "@/lib/utils";

// Version 7 — cloned from Version 6 (AiShellV6) so it can evolve independently.
// Difference vs V6: [future variations to be implemented]

let idCounter = 0;
const nextId = () => `m7-${++idCounter}`;
  ...
  const handleAction = useCallback(
    (label: string) => {
      if (label === "Add it" && flowId === "cart-recovery" && panel === "home") {
        router.push("/ai7?panel=home&flow=cart-recovery&extended=true");
      }
