...
function AddEl() {
  ...
  return (
    <>
      <button
        type="button"
        onClick={(e) => { e.stopPropagation(); setOpen(true); setPulse(p => p + 1); }}
        onMouseDown={(e) => e.stopPropagation()}
        className="w-10 h-10 rounded-lg bg-white border-2 border-dashed border-gray-300 flex items-center justify-center text-gray-400 hover:border-blue-400 hover:text-blue-500 hover:bg-blue-50 transition-colors shadow-sm"
        title="Add step"
      >
  ...
  return (
    <header className="relative flex items-center h-11 px-4 border-b border-gray-200 bg-white gap-3 flex-shrink-0 shadow-sm z-10">
      <nav className="flex items-center gap-1 text-sm text-gray-500">
        ...
        <span className="text-gray-300">/</span>
        <span className="text-gray-800 font-medium">click-action</span>
...
function FlowCanvas({
  ...
  // Center the flow's content in the viewport at a comfortable zoom.
  const fitView = () => {
    ...
    const fit = Math.min((rect.width * 0.8) / cw, (rect.height * 0.8) / ch);
    const scale = Math.min(0.9, Math.max(0.62, +fit.toFixed(2)));
    const cx = (minX + maxX) / 2, cy = (minY + maxY) / 2;
    setTf({ scale, x: rect.width / 2 - cx * scale, y: rect.height / 2 - cy * scale });
  };

  // Fit-and-center on first mount.
    ...
    <LoopOpenContext.Provider value={onLoopClick ?? null}>
      ...
      >
        ...
        <div className="absolute bottom-5 left-5 z-20 flex flex-col gap-1">
          {([{ lbl: "+", fn: () => zoomBy(0.1) }, { lbl: "−", fn: () => zoomBy(-0.1) }, { lbl: "↺", fn: fitView }] as { lbl: string; fn: () => void }[]).map(b => (
...
export default function ClickActionFlowPage() {
