...
const NODES: FN[] = [
  ...
  { id: "trigger", kind: "trigger", x: 866, y: 40, w: 280, h: 68,
    title: "New Email Matching Search - one", subtitle: "Every 5 minutes",
    badge: "Update available", bc: "blue", icon: "gmail" },
        ...
        <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50 py-1 min-w-[130px]">
          {[
            ...
            <button
              ...
              className={`w-full flex items-center gap-2 px-3 py-1.5 text-[11px] font-medium hover:bg-gray-50 transition-colors ${cls}`}
            >
              {icon}{label}
            </button>
          ))}
...
function VersionDropdown({ currentVersion }: { currentVersion: string }) {
  ...
  return (
    <div className="relative flex-shrink-0" onBlur={(e) => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setOpen(false); }} tabIndex={-1}>
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-1.5 text-xs border border-gray-200 text-gray-600 px-2.5 py-1 rounded font-medium hover:bg-gray-50 transition-colors"
      >
        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
        </svg>
        {selected.v}
        <span className={`text-[9px] font-semibold px-1 py-0.5 rounded leading-none ${selected.active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
          {selected.label}
        </span>
        ...
        <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50 py-1 min-w-[200px]">
          ...
          {VERSIONS.map(ver => (
            <button
              ...
              className={`w-full flex items-center justify-between px-3 py-2 hover:bg-gray-50 transition-colors ${ver.v === currentVersion ? "bg-blue-50" : ""}`}
            >
              <div className="flex items-center gap-2">
  ...
  return (
    <header className="flex items-center h-11 px-4 border-b border-gray-200 bg-white gap-3 flex-shrink-0 shadow-sm z-10">
      <nav className="flex items-center gap-1 text-sm text-gray-500">
        ...
        <span className="ml-1.5 text-[10px] bg-green-500 text-white px-1.5 py-0.5 rounded-full font-semibold leading-none">
          LIVE
        </span>
      </nav>
      <div className="flex items-center gap-2 ml-auto">
        ...
        <div className="w-6 h-6 rounded-full bg-gray-300 flex items-center justify-center text-xs font-semibold text-gray-600 ml-1 flex-shrink-0">
          A
        </div>
      </div>
    </header>
...
export default function FlowPage() {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      >
        {/* Zoom controls — ready for pan/drag/zoom wiring */}
        <div className="absolute bottom-5 left-5 z-20 flex flex-col gap-1">
          {([
            { lbl: "+", fn: () => zoomBy(0.1)  },
            { lbl: "−", fn: () => zoomBy(-0.1) },
            { lbl: "↺", fn: () => setTf({ x: 20, y: 16, scale: 0.58 }) },
          ] as { lbl: string; fn: () => void }[]).map(b => (
            <button
              key={b.lbl}
              onClick={b.fn}
              className="w-8 h-8 bg-white border border-gray-200 rounded-md shadow-sm hover:bg-gray-50 flex items-center justify-center text-gray-600 text-sm font-medium transition-colors"
            >
              {b.lbl}
            </button>
          ))}
        </div>
        <div className="absolute bottom-5 left-[3.75rem] z-20 bg-white/90 border border-gray-200 rounded-md shadow-sm px-2 py-1 text-[10px] text-gray-500 font-medium pointer-events-none">
          {Math.round(tf.scale * 100)}%
        </div>

        {/* Transformable canvas — translate + scale state is the hook point for future pan/drag */}
        <div
          style={{
        ...
        >
          ...
          >
            {EDGES.map(e => {
              ...
              const sy = a.y + a.h;
              const ex = b.x + b.w / 2;
              const ey = b.kind === "condition" ? b.y + b.h : b.y;
              const lx = (sx + ex) / 2;
              const mid = (sy + ey) / 2;
          ...
          {/* Group nodes first (renders behind regular nodes) */}
          {groups.map(n => (
            <div
              key={n.id}
              data-node-id={n.id}
